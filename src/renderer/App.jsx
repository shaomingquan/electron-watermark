  ...
  return (
    <div>
        ...
        {(state === PROCESSING || state === DONE) && !!task?.taskId && (
          <>
            <Button color="blue" onClick={openTaskDir}>
              已生成文件（{files.filter((f) => f.state === 'done').length}/
              {files.length}）
            </Button>
          </>
        )}
      ...
      {files.length > 0 && (
        <div className="files">
          {files.map((item) => {
            console.log(item.state, getFileStateColor(item.state));
            return (
              <div className="file">
                <span
