...
const makeDrawtext = i => {
    return `drawtext=fontfile=simhei.ttf: text='禁止搬运':x=${0}:y=${i * 220}:fontsize=180:fontcolor=white:alpha=0.5`
}
const drawtext = [
    makeDrawtext(0),
    makeDrawtext(1),
...
export const genTask = task => {
    ...
    function updateTask () {
        ...
        if (_.every(files, file => file.state === 'done' || file.state === 'error')) {
            task.state = 'done'
        }
    }
